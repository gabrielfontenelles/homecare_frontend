...
export default function NovoAgendamentoPage() {
  ...
  // Determina se deve mostrar o campo especialidade
  ...
  // Determina se deve mostrar o campo profissional
  const showProfissional = () => {
    // Para médico, pode ser sem profissional (ida ao médico externo)
    if (form.tipo === "MEDICO") return false
    return true
  }
  ...
  const validate = () => {
    ...
    if (!form.horaInicio) errors.horaInicio = "Horário de início é obrigatório."
    if (!form.horaFim) errors.horaFim = "Horário de fim é obrigatório."
    if (!form.tipo) errors.tipo = "Tipo é obrigatório."
    if (!form.modalidade) errors.modalidade = "Modalidade é obrigatória."
    if (showEspecialidade() && !form.especialidade) errors.especialidade = "Especialidade é obrigatória."
    if (showProfissional() && !form.profissionalId && !form.nomeProfissional) errors.nomeProfissional = "Informe o nome do profissional externo."
    return errors
  }
      ...
      <Card className="w-full max-w-2xl bg-background dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="button" variant="outline" className="w-full mt-2" onClick={() => router.push("/dashboard/agendamentos")}>Cancelar</Button>
          </form>
        </CardContent>
      </Card>
    </div>
